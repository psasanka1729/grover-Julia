...
#Do the git-ing
cmd='git commit -a -m "Commit before run 3_energy_index" > /dev/null'
print(cmd)
...
open('3_energy_index.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('3_energy_index.task','w')
template_file='3_energy_index.template'
...
for L in xrange(32):
	...
	task_file.write('bash 3_energy_index_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('3_energy_index.sbatch.template','r').read()
...
open('3_energy_index.sbatch','w').write(contents)
