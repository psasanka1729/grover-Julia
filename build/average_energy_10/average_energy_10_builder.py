...
#Do the git-ing
cmd='git commit -a -m "Commit before run average_energy_10" > /dev/null'
print(cmd)
...
open('average_energy_10.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('average_energy_10.task','w')
template_file='average_energy_10.template'
...
for L in xrange(32):
	...
	task_file.write('bash average_energy_10_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('average_energy_10.sbatch.template','r').read()
...
open('average_energy_10.sbatch','w').write(contents)
