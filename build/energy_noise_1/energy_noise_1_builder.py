...
#Do the git-ing
cmd='git commit -a -m "Commit before run energy_noise_1" > /dev/null'
print(cmd)
...
open('energy_noise_1.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('energy_noise_1.task','w')
template_file='energy_noise_1.template'
...
for L in xrange(M):
	...
	task_file.write('bash energy_noise_1_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('energy_noise_1.sbatch.template','r').read()
...
open('energy_noise_1.sbatch','w').write(contents)
