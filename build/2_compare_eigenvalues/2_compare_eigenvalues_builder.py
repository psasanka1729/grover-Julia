...
#Do the git-ing
cmd='git commit -a -m "Commit before run 2_compare_eigenvalues" > /dev/null'
print(cmd)
...
open('2_compare_eigenvalues.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('2_compare_eigenvalues.task','w')
template_file='2_compare_eigenvalues.template'
...
for L in xrange(16):
	...
	task_file.write('bash 2_compare_eigenvalues_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('2_compare_eigenvalues.sbatch.template','r').read()
...
open('2_compare_eigenvalues.sbatch','w').write(contents)
