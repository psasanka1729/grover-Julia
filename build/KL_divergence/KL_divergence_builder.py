...
#Do the git-ing
cmd='git commit -a -m "Commit before run KL_divergence" > /dev/null'
print(cmd)
...
open('KL_divergence.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('KL_divergence.task','w')
template_file='KL_divergence.template'
...
for L in xrange(32):
	...
	contents=contents.replace('*222*',str(20000+L))
	vmap_file.write(str(vnum)+'\t'+str(L)+'\n')
	task_file.write('bash KL_divergence_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('KL_divergence.sbatch.template','r').read()
...
open('KL_divergence.sbatch','w').write(contents)
