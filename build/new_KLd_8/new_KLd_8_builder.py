...
#Do the git-ing
cmd='git commit -a -m "Commit before run new_KLd_8" > /dev/null'
print(cmd)
...
open('new_KLd_8.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('new_KLd_8.task','w')
template_file='new_KLd_8.template'
...
for L in xrange(256):
	...
	task_file.write('bash new_KLd_8_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('new_KLd_8.sbatch.template','r').read()
...
open('new_KLd_8.sbatch','w').write(contents)
