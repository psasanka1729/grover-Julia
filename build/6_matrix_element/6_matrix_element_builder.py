...
#Do the git-ing
cmd='git commit -a -m "Commit before run 6_matrix_element" > /dev/null'
print(cmd)
...
open('6_matrix_element.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('6_matrix_element.task','w')
template_file='6_matrix_element.template'
...
for L in xrange(16):
	...
	task_file.write('bash 6_matrix_element_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('6_matrix_element.sbatch.template','r').read()
...
open('6_matrix_element.sbatch','w').write(contents)
