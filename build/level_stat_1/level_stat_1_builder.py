...
#Do the git-ing
cmd='git commit -a -m "Commit before run level_stat_1" > /dev/null'
print(cmd)
...
open('level_stat_1.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('level_stat_1.task','w')
template_file='level_stat_1.template'
...
contents=open('level_stat_1.sbatch.template','r').read()
...
open('level_stat_1.sbatch','w').write(contents)
