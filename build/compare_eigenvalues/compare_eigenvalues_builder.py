...
#Do the git-ing
cmd='git commit -a -m "Commit before run compare_eigenvalues" > /dev/null'
print(cmd)
...
open('compare_eigenvalues.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('compare_eigenvalues.task','w')
template_file='compare_eigenvalues.template'
...
for L in xrange(32):
	...
	task_file.write('bash compare_eigenvalues_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('compare_eigenvalues.sbatch.template','r').read()
...
open('compare_eigenvalues.sbatch','w').write(contents)
