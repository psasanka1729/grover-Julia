...
#Do the git-ing
cmd='git commit -a -m "Commit before run 2_level_stat" > /dev/null'
print(cmd)
...
open('2_level_stat.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('2_level_stat.task','w')
template_file='2_level_stat.template'
...
for L in xrange(16):
	...
	contents=contents.replace('*111*',str(1600+L))
	out_file_base='data_'+str(L)+'_*lll*.out'
	contents=contents.replace('*jjj*',out_file_base.replace('*lll*','julia'))
	vmap_file.write(str(vnum)+'\t'+str(L)+'\n')
	task_file.write('bash 2_level_stat_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('2_level_stat.sbatch.template','r').read()
...
open('2_level_stat.sbatch','w').write(contents)
