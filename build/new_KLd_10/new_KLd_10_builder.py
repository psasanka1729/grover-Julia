...
#Do the git-ing
cmd='git commit -a -m "Commit before run new_KLd_10" > /dev/null'
print(cmd)
...
open('new_KLd_10.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('new_KLd_10.task','w')
template_file='new_KLd_10.template'
...
for L in xrange(128):
	...
	task_file.write('bash new_KLd_10_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('new_KLd_10.sbatch.template','r').read()
...
open('new_KLd_10.sbatch','w').write(contents)
