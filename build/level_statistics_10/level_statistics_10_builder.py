...
#Do the git-ing
cmd='git commit -a -m "Commit before run level_statistics_10" > /dev/null'
print(cmd)
...
open('level_statistics_10.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('level_statistics_10.task','w')
template_file='level_statistics_10.template'
...
for L in xrange(32):
	...
	task_file.write('bash level_statistics_10_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('level_statistics_10.sbatch.template','r').read()
...
open('level_statistics_10.sbatch','w').write(contents)
