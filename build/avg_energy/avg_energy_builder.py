...
#Do the git-ing
cmd='git commit -a -m "Commit before run avg_energy" > /dev/null'
print(cmd)
...
open('avg_energy.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('avg_energy.task','w')
template_file='avg_energy.template'
...
for L in xrange(32):
	...
	vmap_file.write(str(vnum)+'\t'+str(2000+2000+L)+'\n')
	task_file.write('bash avg_energy_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('avg_energy.sbatch.template','r').read()
...
open('avg_energy.sbatch','w').write(contents)
