...
#Do the git-ing
cmd='git commit -a -m "Commit before run matrix_element" > /dev/null'
print(cmd)
...
open('matrix_element.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('matrix_element.task','w')
template_file='matrix_element.template'
...
contents=open('matrix_element.sbatch.template','r').read()
...
open('matrix_element.sbatch','w').write(contents)
