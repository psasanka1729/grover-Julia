...
#Do the git-ing
cmd='git commit -a -m "Commit before run delta_E_n" > /dev/null'
print(cmd)
...
open('delta_E_n.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('delta_E_n.task','w')
template_file='delta_E_n.template'
...
for L in xrange(256):
	...
	task_file.write('bash delta_E_n_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('delta_E_n.sbatch.template','r').read()
...
open('delta_E_n.sbatch','w').write(contents)
