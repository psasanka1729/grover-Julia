...
#Do the git-ing
cmd='git commit -a -m "Commit before run energy_noise_2" > /dev/null'
print(cmd)
...
open('energy_noise_2.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('energy_noise_2.task','w')
template_file='energy_noise_2.template'
...
contents=open('energy_noise_2.sbatch.template','r').read()
...
open('energy_noise_2.sbatch','w').write(contents)
