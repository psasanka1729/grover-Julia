...
#Do the git-ing
cmd='git commit -a -m "Commit before run average_energy" > /dev/null'
print(cmd)
...
open('average_energy.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('average_energy.task','w')
template_file='average_energy.template'
...
for L in xrange(16):
	...
	task_file.write('bash average_energy_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('average_energy.sbatch.template','r').read()
...
open('average_energy.sbatch','w').write(contents)
