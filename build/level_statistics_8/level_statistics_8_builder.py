...
#Do the git-ing
cmd='git commit -a -m "Commit before run level_statistics_8" > /dev/null'
print(cmd)
...
open('level_statistics_8.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('level_statistics_8.task','w')
template_file='level_statistics_8.template'
...
contents=open('level_statistics_8.sbatch.template','r').read()
...
open('level_statistics_8.sbatch','w').write(contents)
