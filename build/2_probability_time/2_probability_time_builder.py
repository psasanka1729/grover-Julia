...
#Do the git-ing
cmd='git commit -a -m "Commit before run 2_probability_time" > /dev/null'
print(cmd)
...
open('2_probability_time.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('2_probability_time.task','w')
template_file='2_probability_time.template'
template_contents=open(template_file,'r').read()

vnum=0

deltas = [0.0,0.05,0.1,0.15,0.17,0.18,0.2,0.22,0.23,0.25,0.3]
for L in xrange(11):
	...
	task_file.write('bash 2_probability_time_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('2_probability_time.sbatch.template','r').read()
...
open('2_probability_time.sbatch','w').write(contents)
