...
#Do the git-ing
cmd='git commit -a -m "Commit before run avg_energy_2" > /dev/null'
print(cmd)
...
open('avg_energy_2.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('avg_energy_2.task','w')
template_file='avg_energy_2.template'
...
for L in xrange(64):
	...
	task_file.write('bash avg_energy_2_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('avg_energy_2.sbatch.template','r').read()
...
open('avg_energy_2.sbatch','w').write(contents)
