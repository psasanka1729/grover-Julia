...
#Do the git-ing
cmd='git commit -a -m "Commit before run matrix_data_heff" > /dev/null'
print(cmd)
...
open('matrix_data_heff.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('matrix_data_heff.task','w')
template_file='matrix_data_heff.template'
...
for L in xrange(16):
	...
	task_file.write('bash matrix_data_heff_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('matrix_data_heff.sbatch.template','r').read()
...
open('matrix_data_heff.sbatch','w').write(contents)
