...
#Do the git-ing
cmd='git commit -a -m "Commit before run 1_energy_index" > /dev/null'
print(cmd)
...
open('1_energy_index.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('1_energy_index.task','w')
template_file='1_energy_index.template'
...
contents=open('1_energy_index.sbatch.template','r').read()
...
open('1_energy_index.sbatch','w').write(contents)
