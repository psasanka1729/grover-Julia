...
#Do the git-ing
cmd='git commit -a -m "Commit before run run2_03092022" > /dev/null'
print(cmd)
...
open('run2_03092022.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('run2_03092022.task','w')
template_file='run2_03092022.template'
...
for L in xrange(2):
	...
	task_file.write('bash run2_03092022_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('run2_03092022.sbatch.template','r').read()
...
open('run2_03092022.sbatch','w').write(contents)
