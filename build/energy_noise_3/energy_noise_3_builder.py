...
#Do the git-ing
cmd='git commit -a -m "Commit before run energy_noise_3" > /dev/null'
print(cmd)
...
open('energy_noise_3.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('energy_noise_3.task','w')
template_file='energy_noise_3.template'
...
for L in xrange(16):
	...
	task_file.write('bash energy_noise_3_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('energy_noise_3.sbatch.template','r').read()
...
open('energy_noise_3.sbatch','w').write(contents)
