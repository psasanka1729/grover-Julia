...
#Do the git-ing
cmd='git commit -a -m "Commit before run level_stat_energy_1" > /dev/null'
print(cmd)
...
open('level_stat_energy_1.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('level_stat_energy_1.task','w')
template_file='level_stat_energy_1.template'
...
for L in xrange(32):
	...
	contents=contents.replace('*111*',str(4000+L))
	vmap_file.write(str(vnum)+'\t'+str(L)+'\n')
	task_file.write('bash level_stat_energy_1_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('level_stat_energy_1.sbatch.template','r').read()
...
open('level_stat_energy_1.sbatch','w').write(contents)
