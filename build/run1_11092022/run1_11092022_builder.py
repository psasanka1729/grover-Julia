...
#Do the git-ing
cmd='git commit -a -m "Commit before run run1_11092022" > /dev/null'
print(cmd)
...
open('run1_11092022.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('run1_11092022.task','w')
template_file='run1_11092022.template'
...
for L in xrange(1):
	...
	task_file.write('bash run1_11092022_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('run1_11092022.sbatch.template','r').read()
...
open('run1_11092022.sbatch','w').write(contents)
