...
#Do the git-ing
cmd='git commit -a -m "Commit before run run5_03092022" > /dev/null'
print(cmd)
...
open('run5_03092022.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('run5_03092022.task','w')
template_file='run5_03092022.template'
template_contents=open(template_file,'r').read()

vnum=0

deltas = [0.1,0.12,0.13,0.14,0.15,0.16,0.17,0.18,0.19,0.2,0.22,0.24,0.26,0.28,0.3,0.4]
for L in xrange(16):
	...
	task_file.write('bash run5_03092022_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('run5_03092022.sbatch.template','r').read()
...
open('run5_03092022.sbatch','w').write(contents)
